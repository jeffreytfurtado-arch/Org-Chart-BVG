// Shared org-chart state, stored server-side in Netlify Blobs.
...
exports.handler = async (event) => {
  ...
  let store;
  try { store = blobStore("orgchart"); } catch (e) { return jsonResp(500, { error: "store" }); }

  if (event.httpMethod === "GET") {
    let cur = null;
    try { cur = await store.get("state", { type: "json" }); } catch {}
    if (cur && Array.isArray(cur.employees)) {
      return jsonResp(200, { state: { employees: cur.employees, secondaryLinks: cur.secondaryLinks || [] }, rev: cur.rev || 0, canEdit });
    }
    return jsonResp(200, { state: null, rev: 0, canEdit });
  }
  ...
  if (event.httpMethod === "POST") {
    ...
    const secondaryLinks = body.secondaryLinks || [];
    if (!Array.isArray(employees)) return jsonResp(400, { error: "employees must be array" });
    let cur = null;
    try { cur = await store.get("state", { type: "json" }); } catch {}
    const rev = (cur && cur.rev) ? cur.rev + 1 : 1;
    await store.setJSON("state", { employees, secondaryLinks, rev, updatedBy: email, updatedAt: new Date().toISOString() });
    // Read-after-write verification
    let verified = false;
    try {
      const check = await store.get("state", { type: "json" });
      verified = check && check.rev === rev;
      if (!verified) console.error("[state.js] WRITE NOT VERIFIED! expected rev=", rev, "got=", check && check.rev);
    } catch (ve) { console.error("[state.js] verify read failed:", ve); }
    return jsonResp(200, { ok: true, rev, verified });
  }
