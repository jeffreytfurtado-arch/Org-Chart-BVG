...
export default async (req, context) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (!raw) {
        return json({ data: { domains: [], processes: [] }, rev: 0 });
      }
      return json({
        data: raw.data || { domains: [], processes: [] },
        rev: raw.rev || 0,
      });
    }
    ...
    if (req.method === 'POST') {
      ...
      const { domains, processes, baseRev } = body;
      ...
      await store.set('data', JSON.stringify({
        data: { domains: domains || [], processes: processes || [] },
        rev: newRev,
      }));
    ...
    console.error('raci function error:', e);
