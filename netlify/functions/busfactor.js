// Bus-factor data, stored server-side in Netlify Blobs.
...
exports.handler = async (event) => {
  ...
  let store;
  try { store = blobStore("busfactor"); } catch (e) { return jsonResp(500, { error: "store" }); }

  if (event.httpMethod === "GET") {
    let cur = null;
    try { cur = await store.get("data", { type: "json" }); } catch {}
    if (cur && Array.isArray(cur.entries)) {
      return jsonResp(200, { data: { entries: cur.entries }, rev: cur.rev || 0, canEdit });
    }
    return jsonResp(200, { data: { entries: [] }, rev: 0, canEdit });
  }
  ...
  if (event.httpMethod === "POST") {
    ...
    if (!Array.isArray(entries)) return jsonResp(400, { error: "entries must be array" });
    let cur = null;
    try { cur = await store.get("data", { type: "json" }); } catch {}
    ...
    const rev = curRev + 1;
    await store.setJSON("data", { entries, rev, updatedBy: email, updatedAt: new Date().toISOString() });
